...
Once at least two players have joined, any player (the ``host'') can start the match.
...
\begin{itemize}
    ...
    \item \textbf{Fault tolerance} ensures that the match continues uninterrupted even if the primary server crashes: a hot standby backup takes over transparently.
\end{itemize}
...
\begin{enumerate}
    ...
    \item[\textbf{NF4}] \textbf{Transparency}: clients shall be unaware of the primary/backup topology; failover shall be invisible from the client's perspective.
    \item[\textbf{NF5}] \textbf{Failover reconnect}: after a server failover, the proxy shall transparently restore all active client sessions to the new primary without requiring manual reconnection.
\end{enumerate}
...
The thread-per-client model is simple but does not scale beyond a few dozen connections; this is acceptable given the four-player limit.

\paragraph{Scalability.}
The system is intentionally limited in scale: at most four players by game design, with an unbounded number of spectators in principle but constrained in practice by server resources.
No horizontal scaling is implemented or required for the target use case.

\paragraph{Resource sharing.}
All clients share a single \texttt{GameService} instance on the server, which holds the authoritative game state.
The proxy acts as a single entry point, concentrating all client traffic through one well-known address.

\paragraph{Openness.}
Communication uses TCP with line-delimited JSON payloads, both widely supported and human-readable.
The message schema is defined in \texttt{src/common/constants.py}; any client implementing that schema can connect to the server, independent of language or platform.

\paragraph{Evolvability.}
State snapshots carry a \texttt{"version"} field so that incompatible schema changes can be detected at runtime.
The separation into three independent processes (server, proxy, client) with JSON as the interface means components can be replaced or updated without affecting the others, within the constraints of the current protocol.
...
\draw[arr] (GM.south) -- (LV.north);
\draw[arr] (GC.south) -- (GV.north);
\draw[arr] (CN.south) -- (VV.north);
\begin{pgfonlayer}{background}
...
\paragraph{Messages.}
Two classes of messages are exchanged:
\begin{itemize}
...
The test suite is written with \textbf{pytest} \citep{pytest}.
...
\begin{enumerate}
    \item Start server, proxy, and two clients using \texttt{bash start.sh}.
    \item Start a match; verify real-time state synchronisation across clients.
    \item Identify the primary server PID via \texttt{netstat -ano | grep 5556}.
    \item Kill the primary with \texttt{taskkill /PID \{pid\} /F}.
...
Alternatively, start each component manually in separate terminals (server first):
\begin{lstlisting}
# Terminal 1
poetry run bomberman-server
...
A match requires at least two connected players to start.
...
I designed and built the \textbf{transparency layer} — the TCP proxy (\texttt{fault\_tolerance/proxy\_server.py}) and the client network layer (\texttt{network/client\_network.py}) — which hides server failover from clients entirely.
...
The key design decision was to concentrate all reconnection logic in the proxy: the client is completely unaware of failover, and the server only needs to handle a single \texttt{RECONNECT} message per session to restore a client's state.
On the client side, I implemented the \texttt{NetworkManager}, which manages the persistent TCP connection, message framing (line-delimited JSON), and the background receive thread.
I also developed the client MVC structure (\texttt{GameController}, \texttt{GameState}, Pygame views) and the \texttt{start.sh} deployment script, and wrote the client-side tests (\texttt{test\_game.py}, \texttt{NetworkManager} section of \texttt{test\_controller.py}).

A known weakness of the proxy design is that it is itself a single point of failure: if it crashes, no client can communicate with the server regardless of the server cluster's health.
A more robust solution would either replicate the proxy or embed a lightweight reconnection mechanism directly into the client.

\subsection{Gioele Santi}

I owned two areas end to end: the \textbf{fault-tolerance stack} and the \textbf{consistency layer}.

The fault-tolerance side is built around four cooperating components: \texttt{FailureDetector} (heartbeat-based failure detection with a 1.5\,s timeout), \texttt{BackupServer} (state receiver, heartbeat monitor, promotion logic), \texttt{PrimaryServer} (heartbeat responder, state replication sender), and \texttt{AutoSpawner} (subprocess-based backup lifecycle management, including automatic respawn on unexpected termination).
...
For consistency, I implemented the \texttt{GameService} service layer, which is the single component authorized to mutate game state.
All methods are protected by a \texttt{threading.RLock} via the \texttt{@\_synchronized} decorator, ensuring that client handler threads and the game loop thread never produce concurrent writes.
...
A single missing or corrupted field in the codec would silently corrupt the backup's state at promotion time, so I covered it with dedicated round-trip tests (\texttt{test\_state\_codec.py}).
I also wrote \texttt{test\_core.py}, \texttt{test\_game\_service.py}, and the \texttt{CommandController} section of \texttt{test\_controller.py}, and implemented the server network layer (\texttt{network/server\_network.py}), the command dispatcher (\texttt{controller/command\_controller.py}), the domain model (\texttt{models.py}), and the pure game logic (\texttt{core.py}).
