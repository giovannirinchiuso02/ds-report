...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{figures/UML1.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{figures/uml2.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{figures/uml3.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{figures/uml4.png}
...
failure}: if it crashes, or is partitioned from all clients, the service becomes
unavailable even when both servers are healthy. This is a deliberate trade-off.
Unlike the primary, the proxy holds no game state, so its failure severs
connections without losing match data, and concentrating the failover logic in
one stateless component is what keeps the client implementation free of any
reconnection or discovery logic. Removing this limitation would require either
replicating the proxy --- which reintroduces the same coordination problem one
level up --- or pushing reconnection logic into the client, and both options were
...
client that drops voluntarily cannot rejoin, because the \texttt{RECONNECT}
handshake is exercised only by the proxy during failover. Generalising it would
require a per-player grace timer driven by the game loop.
