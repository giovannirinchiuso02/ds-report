...
\begin{itemize}
    ...
    requests a player slot), \texttt{CHAT:\{message\}}, \texttt{PING}.
    Commands are case-insensitive and newline-terminated.    
    \item \textbf{State snapshots} (server $\to$ client): the JSON projection of
    the current state described above, sent every 100\,ms.    \item \textbf{Control messages}: \texttt{RECONNECT:\{session\_id\}} (proxy $\to$ server post-failover), \texttt{HEARTBEAT} / \texttt{ALIVE} (backup $\leftrightarrow$ primary).
\end{itemize}
...
%% 4. movement command
\draw[msg] (\xC,-4.5) -- node[lbl, above]{RIGHT} (\xP,-4.5);
\draw[msg] (\xP,-4.9) -- node[lbl, above]{RIGHT} (\xS,-4.9);
\end{tikzpicture}
