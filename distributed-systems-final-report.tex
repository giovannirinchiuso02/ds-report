...
Start game & Host & Press Enter in the lobby when $\geq 2$ players are connected \\
...
\begin{enumerate}
    ...
    \item[\textbf{F7}] Players and spectators shall be able to send chat messages visible to all participants.
...
All client inputs are serialised through \texttt{GameService}'s single \texttt{RLock}, so there are no concurrent write conflicts.
...
\begin{itemize}
    ...
    \item \textbf{Spectator}: name, join time, connected status, reconnect session ID.
...
Figure~\ref{fig:components} shows the main components of the four running processes --- proxy, primary, backup and client --- and the channels connecting them.
...
\begin{itemize}
    \item \textbf{Consistency}: a single serialisation point (\texttt{GameService}'s \texttt{RLock}) eliminates the possibility of concurrent writes and distributed state divergence.
...
Each tick:
\begin{enumerate}
    \item If the phase is \texttt{victory}, decrement the return-to-lobby timer and skip the rest of the tick.
    \item Decrement bomb timers; detonate expired bombs, compute explosion cells, destroy the blocks they reach and reduce player lives.
    \item Decrement explosion display timers; remove expired explosions.
    \item Check the victory condition; if it is met, stop here.
    \item Decrement the block-regeneration timer; when it expires, place a new destructible block on a safe tile and re-arm the timer with a random interval.
...
\begin{itemize}
  \item \texttt{lobby} $\Rightarrow$ \texttt{LobbyView}: displays connected players, spectators, and the ``Press ENTER'' start prompt (host only).
...
\begin{enumerate}
    ...
    \item Identify the primary server PID via \texttt{netstat -ano | findstr 5556 | findstr LISTENING} (Windows) or \texttt{ss -tlnp | grep 5556} (Linux/macOS).
...
5558 & Reserved (\texttt{BACKUP\_GAME\_PORT}); not bound by the current implementation \\
...
The host (the first player to join) sees a ``Press ENTER'' prompt.
The game starts when the host presses Enter with at least two players connected.
The interface is driven entirely from the keyboard: the client handles no mouse input at all.
...
Destroyed blocks are periodically regenerated on tiles that are safe to occupy, so the map does not grow progressively emptier over a long match.
...
The victory screen displays the winner and a countdown before returning all clients to the lobby automatically; a player can also press Enter to request a rematch immediately (\texttt{PLAY\_AGAIN}).
...
It does not preserve a match in progress, for the reasons discussed in \cref{availability}.
...
A limitation of the current replication design, discussed in \cref{interaction}, is opening a new TCP connection for every snapshot rather than maintaining a persistent channel --- functional at the current scale, but worth revisiting if the replication interval were reduced or the state size increased significantly.
...
and safe spawn zones, bomb placement and explosion propagation (a cross-shaped
blast of radius 2, stopped by walls and consuming the first destructible block on
each arm), player lifecycle management, the lobby/playing/victory state machine,
timed regeneration of destroyed blocks, and the spectator system. It was later
